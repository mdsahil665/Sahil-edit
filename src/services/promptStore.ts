...
class PromptStore {
  ...
  private initLocalStorageCache() {
    try {
      ...
      this.postsCache = storedPosts ? JSON.parse(storedPosts) : INITIAL_PROMPTS;
      ...
      this.postsCache = INITIAL_PROMPTS;
      ...
      async (snapshot) => {
        if (snapshot.empty && !this.isInitialized) {
          ...
          snapshot.forEach((docSnap) => {
            postsList.push({ id: docSnap.id, ...docSnap.data() } as PromptPost);
          });
          postsList.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
          this.postsCache = postsList;
          this.saveLocalCache();
          this.notify();
        }
      },
  ...
  public async addPost(
    ...
    try {
      ...
      await setDoc(doc(db, 'prompts', id), dataToSave);
  ...
  public async updatePost(id: string, updates: Partial<PromptPost>): Promise<PromptPost | null> {
    ...
    try {
      const { id: _, ...dataToSave } = updatedPost;
      if (dataToSave.imageUrl && dataToSave.imageUrl.startsWith('data:image/') && dataToSave.imageUrl.length > 500000) {
        dataToSave.imageUrl = 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?auto=format&fit=crop&w=1200&q=80';
      }
      await setDoc(doc(db, 'prompts', id), dataToSave);
